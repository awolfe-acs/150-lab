import{l as t}from"./logger-2Ii2FPkr.js";const ae=document.querySelector('script[src*="/content/dam/acsorg/150/"]')!==null,ne=window.location.hostname.includes("github.io")||window.location.pathname.startsWith("/150-lab/"),J=ae?"/content/dam/acsorg/150/assets":ne?"/150-lab/assets":"",D=/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)||navigator.maxTouchPoints&&navigator.maxTouchPoints>2||window.matchMedia("(max-width: 768px)").matches,Q=D?"acs-150-compressed-4.mp4":"acs-150-compressed-3.mp4",K=`${J}/video/${Q}`,se=`${J}/images/ACS150-promo-cover.jpg`,de=/^((?!chrome|android).)*safari/i.test(navigator.userAgent),le=/iPad|iPhone|iPod/.test(navigator.userAgent)&&!window.MSStream,Z=de||le;console.log("[video.js] Device detection:",{isMobile:D,isSafariOrIOS:Z,videoFileName:Q});let F=!1,b=null,N=!1,k=!1;function ue(i){try{if(k){t.log("YouTube player already initialized, skipping"),i&&i();return}if(window.YT&&window.YT.Player){t.log("YouTube API already loaded, initializing player immediately"),x(),i&&i();return}if(window.onYouTubeIframeAPIReady){t.log("YouTube API is loading, chaining our callback");const e=window.onYouTubeIframeAPIReady;window.onYouTubeIframeAPIReady=()=>{try{typeof e=="function"&&e()}catch(l){t.warn("Error in chained onYouTubeIframeAPIReady callback:",l)}k||x(),i&&i()};return}t.log("Loading YouTube API for the first time");const s=document.createElement("script");s.src="https://www.youtube.com/iframe_api";const g=document.getElementsByTagName("script")[0];g&&g.parentNode?g.parentNode.insertBefore(s,g):document.head.appendChild(s),window.onYouTubeIframeAPIReady=()=>{t.log("YouTube API ready, initializing player"),k||x(),i&&i()}}catch(s){t.error("Error in initYouTubeAPI:",s),i&&i()}}function ce(){if(N){t.log("YouTube iframe already loaded, skipping");return}const i=document.getElementById("youtube-video-iframe");if(i){if(i.src&&i.src!=="about:blank"&&i.src!==""){t.log("YouTube iframe already has src, marking as loaded"),N=!0;return}i.dataset.src&&(t.log("Lazy loading YouTube iframe from data-src"),i.src=i.dataset.src,N=!0)}}let Y=!1;function x(){if(k){t.log("YouTube player already initialized (initPlayer check), skipping");return}const i=document.getElementById("youtube-video-iframe");if(!i||!document.body.contains(i)){t.warn("YouTube iframe not found or not in DOM, delaying player init"),setTimeout(x,100);return}if(!i.src||i.src==="about:blank"){t.warn("YouTube iframe src not set, delaying player init"),setTimeout(x,100);return}k=!0,i.setAttribute("data-player-managed","true"),i.setAttribute("data-yt-player-initialized","true"),window.mainYouTubePlayerManaged=!0,window.mainYouTubePlayerInitializing=!0;try{b=new YT.Player("youtube-video-iframe",{events:{onReady:ge,onStateChange:me}}),window.isMainVideoPlaying=()=>{try{return Y&&b&&b.getPlayerState&&b.getPlayerState()===1}catch{return!1}},window.isMainYouTubePlayerReady=()=>Y}catch(s){t.error("Error creating YouTube player:",s),k=!1,window.mainYouTubePlayerManaged=!1}}function P(){const i=document.getElementById("youtube-video-wrapper");if(!i)return;i.querySelectorAll("div:not(.video-start-overlay):not(.play-button-overlay)").forEach(g=>{const e=g.style;e.position==="absolute"&&e.top==="0px"&&e.left==="0px"&&e.width==="100%"&&e.height==="100%"&&!g.classList.length&&(t.log("Removing YouTube blocking div"),g.remove())})}function ge(i){t.log("YouTube player ready"),Y=!0,window.mainYouTubePlayerInitializing=!1,window.mainYouTubePlayerReady=!0,P(),setTimeout(P,500),setTimeout(P,1e3)}function me(i){if(!Y){t.warn("Player state change received but player not ready yet");return}t.log("Main video state changed:",i.data),i.data===1?(P(),t.log("[video.js] Video playing, pausing background audio"),window.cancelActiveFade&&window.cancelActiveFade(),window.backgroundAudio&&(window.fadeBackgroundAudio?window.fadeBackgroundAudio(.001,1e3,()=>{t.log("[video.js] Background audio ducked (vol 0.001)")}):window.backgroundAudio.volume=.001)):(i.data===0||i.data===2)&&(t.log("[video.js] Video paused/ended, calling resumeBackgroundAudio"),window.resumeBackgroundAudio?window.resumeBackgroundAudio():t.error("[video.js] resumeBackgroundAudio not found on window"))}function U(i,s,g=1e3){if(!i)return;const e=i.volume,l=performance.now(),A=n=>{const S=n-l,h=Math.min(S/g,1),M=h*h;i.volume=e+(s-e)*M,h<1&&requestAnimationFrame(A)};requestAnimationFrame(A)}function fe(){const i=document.querySelector("#video"),s=document.querySelector(".video-wrapper");if(!i||!s)return;if(s.querySelector("iframe#youtube-video-iframe")){t.log("YouTube iframe detected, setting up lazy loading");const o=document.querySelector(".video-start-overlay");o&&o.addEventListener("click",function m(a){a.preventDefault(),a.stopPropagation();try{if(!window.audioMuted&&window.playUIClickSound)try{window.playUIClickSound()}catch(d){t.warn("Could not play UI click sound:",d)}t.log("[video.js] Overlay clicked, fading out audio immediately");try{window.cancelActiveFade&&window.cancelActiveFade(),window.fadeBackgroundAudio?window.fadeBackgroundAudio(.001,1e3,()=>{t.log("[video.js] Background audio ducked (vol 0.001) via overlay click")}):window.backgroundAudio&&(window.backgroundAudio.volume=.001)}catch(d){t.warn("Audio fade error (non-critical):",d)}o.classList.add("hidden"),ce();const c=document.getElementById("youtube-video-iframe");if(c){const d=()=>{try{ue(()=>{t.log("[video.js] YouTube player initialized after lazy load"),P()})}catch(re){t.error("YouTube player init error:",re)}};c.src&&c.contentWindow?setTimeout(d,200):(c.addEventListener("load",d,{once:!0}),setTimeout(d,1e3))}setTimeout(()=>{try{o.remove(),P()}catch(d){t.warn("Overlay remove error:",d)}},400),setTimeout(()=>{try{window.lenis&&typeof window.lenis.start=="function"&&(window.lenis.start(),t.log("[video.js] Ensured Lenis is running after video click")),window.ScrollTrigger&&typeof window.ScrollTrigger.refresh=="function"&&(window.ScrollTrigger.refresh(),t.log("[video.js] Refreshed ScrollTrigger after video click"))}catch(d){t.warn("Scroll safeguard error:",d)}},500)}catch(c){t.error("Video overlay click error (caught to prevent page break):",c),setTimeout(()=>{try{window.lenis&&typeof window.lenis.start=="function"&&window.lenis.start(),window.ScrollTrigger&&typeof window.ScrollTrigger.refresh=="function"&&window.ScrollTrigger.refresh()}catch{}},100)}o.removeEventListener("click",m)});const r=document.querySelector("#video");r&&new IntersectionObserver(a=>{a.forEach(c=>{if(!c.isIntersecting&&b){t.log("[video.js] Main video scrolled out of view, pausing");try{b.getPlayerState()===1&&b.pauseVideo()}catch(d){t.warn("[video.js] Could not pause video:",d)}setTimeout(()=>{!(window.isMainVideoPlaying?window.isMainVideoPlaying():!1)&&window.resumeBackgroundAudio&&!window.audioMuted&&window.resumeBackgroundAudio()},300)}})},{threshold:.25}).observe(r);return}const e=s.querySelector("video");if(!e){t.log("No <video> element found in .video-wrapper, skipping custom video controls");return}t.log("Custom .mp4 video player detected, initializing controls"),window.isMainVideoPlaying=()=>{try{return e&&!e.paused}catch{return!1}},e.setAttribute("playsinline",""),e.setAttribute("webkit-playsinline",""),Z&&e.setAttribute("x-webkit-airplay","allow"),e.src=K,e.poster=se,e.load(),t.log("[video.js] Video source set:",K,"| Mobile:",D),e.addEventListener("error",o=>{t.error("Video loading error:",o),t.error("Video src:",e.src),t.error("Video error code:",e.error?.code),t.error("Video error message:",e.error?.message)}),e.addEventListener("loadeddata",()=>{t.log("[video.js] Video data loaded successfully"),e.style.opacity="1",e.pause()}),e.addEventListener("loadedmetadata",()=>{t.log("[video.js] Video metadata loaded"),e.style.display="none",e.offsetHeight,e.style.display=""});const l=document.createElement("div");l.className="video-overlay";const A=document.createElement("div");A.className="play-button",l.appendChild(A),e.parentNode.insertBefore(l,e.nextSibling);const n=document.createElement("div");n.className="video-audio-slider",n.innerHTML=`
  ...
  `,n.style.cssText=`
    ...
    color: white;
    font-size: 14px;
  `;const S=n.querySelector(".audio-slider-track");S.style.cssText=`
    width: 80px;
    height: 4px;
  ...
  `;const M=n.querySelector(".audio-slider-thumb");M.style.cssText=`
    position: absolute;
    top: 50%;
    left: 50%;
  ...
  `;const ee=n.querySelector(".audio-icon");ee.style.cssText=`
    display: flex;
    align-items: center;
  ...
  `;const T=u.querySelector(".progress-bar-fill");T.style.cssText=`
    ...
    pointer-events: none;
  `;const f=u.querySelector(".progress-bar-thumb");f.style.cssText=`
    position: absolute;
    top: 50%;
    left: 0%;
    transform: translate(-50%, -50%);
    ...
    pointer-events: none;
  `,e.parentNode.appendChild(u);let V=!1;const X=.3,H=.18,y=()=>{const r=e.volume/X*100;h.style.width=r+"%",M.style.left=r+"%"},R=o=>{const r=S.getBoundingClientRect(),a=Math.max(0,Math.min(100,(o-r.left)/r.width*100))/100*X;if(window.audioMuted&&a>0){const c=document.querySelector(".sound-toggle");c&&c.classList.contains("muted")&&(F=!0,c.click(),setTimeout(()=>{F=!1},100))}a>0?e.muted=!1:e.muted=!0,e.volume=a,a>0&&(L=a),y()};S.addEventListener("mousedown",o=>{V=!0,R(o.clientX),o.preventDefault()}),document.addEventListener("mousemove",o=>{V&&R(o.clientX)}),document.addEventListener("mouseup",()=>{V=!1}),s.addEventListener("mouseenter",()=>{e.paused||(n.style.opacity="1",n.style.pointerEvents="auto")}),s.addEventListener("mouseleave",()=>{n.style.opacity="0",n.style.pointerEvents="none"}),e.addEventListener("volumechange",y),e.volume=H,window.audioMuted?(e.muted=!0,e.volume=0):e.muted=!1,y();let w=!1;const j=()=>{if(e.duration&&!w){const o=e.currentTime/e.duration*100;T.style.transition="none",T.style.width=o+"%",f.style.left=o+"%"}},q=o=>{const r=I.getBoundingClientRect(),a=Math.max(0,Math.min(100,(o-r.left)/r.width*100))/100*e.duration;e.currentTime=a,j(),e.paused||E()},W=()=>{T.style.transition="none",f.style.transition="opacity 0.2s"},_=()=>{T.style.transition="width 0.1s linear",f.style.transition="opacity 0.2s"};I.addEventListener("mousedown",o=>{w=!0,W(),q(o.clientX),o.preventDefault()}),I.addEventListener("click",o=>{w||(W(),q(o.clientX),setTimeout(()=>{_()},50))}),document.addEventListener("mousemove",o=>{w&&q(o.clientX)}),document.addEventListener("mouseup",()=>{w=!1,_()}),u.addEventListener("mouseenter",()=>{f.style.opacity="1",u.style.height="8px",u.style.background="rgba(0, 0, 0, 0.3)"}),u.addEventListener("mouseleave",()=>{w||(f.style.opacity="0"),u.style.height="4px",u.style.background="rgba(0, 0, 0, 0)"});let v=null,C=0;const $=()=>{if(e.duration&&!w&&!e.paused){const o=performance.now();if(o-C>=16.67){const r=e.currentTime/e.duration*100;T.style.width=r+"%",f.style.left=r+"%",C=o}v=requestAnimationFrame($)}},E=()=>{v&&cancelAnimationFrame(v),C=performance.now(),v=requestAnimationFrame($)},te=()=>{v&&(cancelAnimationFrame(v),v=null)};e.addEventListener("play",E),e.addEventListener("pause",te),e.addEventListener("timeupdate",j),j();let L=H,p=null;const oe=()=>{e.paused||(L=e.volume,U(e,0,600),p=setTimeout(()=>{e.pause(),p=null},600))},ie=()=>{e.paused||(p&&(clearTimeout(p),p=null),e.pause())},z=()=>{window.audioMuted?(e.volume=0,e.muted=!0):(e.muted=!1,e.volume=L);const o=e.play();o!==void 0?o.then(()=>{t.log("[video.js] Video playback started successfully"),l.classList.add("hidden"),y(),E()}).catch(r=>{t.warn("[video.js] Play failed:",r.name,r.message),t.log("[video.js] Trying muted playback fallback..."),e.muted=!0,e.volume=0;const m=e.play();m!==void 0&&m.then(()=>{t.log("[video.js] Muted playback started, video is now playing"),l.classList.add("hidden"),y(),E(),window.audioMuted||setTimeout(()=>{try{e.muted=!1,e.volume=L,y(),t.log("[video.js] Successfully unmuted video")}catch(a){t.warn("[video.js] Could not unmute:",a)}},100)}).catch(a=>{t.error("[video.js] Even muted playback failed:",a.name,a.message),l.classList.remove("hidden")})}):(l.classList.add("hidden"),y(),E())},G=()=>{if(t.log("[video.js] handlePlayPause called, paused:",e.paused,"readyState:",e.readyState),e.paused){if(p&&(clearTimeout(p),p=null),window.backgroundAudio&&U(window.backgroundAudio,0),e.readyState<3){t.log("[video.js] Video not loaded yet (readyState:",e.readyState,"), loading within user gesture..."),e.load();const o=()=>{t.log("[video.js] Video canplay fired, attempting playback"),e.removeEventListener("canplay",o),e.removeEventListener("canplaythrough",o),z()};e.addEventListener("canplay",o,{once:!0}),e.addEventListener("canplaythrough",o,{once:!0}),setTimeout(()=>{e.paused&&e.readyState>=2&&(t.log("[video.js] Timeout fallback: attempting play with readyState:",e.readyState),z())},500);return}z()}else ie()};l.addEventListener("click",G),e.addEventListener("click",G),e.addEventListener("ended",()=>{l.classList.remove("hidden"),n.style.opacity="0",n.style.pointerEvents="none",!window.audioMuted&&window.resumeBackgroundAudio&&(t.log("[video.js] Video ended, resuming background audio"),window.resumeBackgroundAudio())}),e.addEventListener("pause",()=>{l.classList.remove("hidden"),n.style.opacity="0",n.style.pointerEvents="none",!window.audioMuted&&window.resumeBackgroundAudio&&(t.log("[video.js] Video paused, resuming background audio"),window.resumeBackgroundAudio())}),new IntersectionObserver(o=>{o.forEach(r=>{if(!r.isIntersecting){const m=!e.paused;oe(),m&&(t.log("[video.js] Custom video left viewport after playing, ensuring background audio resumes"),setTimeout(()=>{!(window.isMainVideoPlaying?window.isMainVideoPlaying():!1)&&window.resumeBackgroundAudio&&!window.audioMuted&&window.resumeBackgroundAudio()},800))}})},{threshold:.5}).observe(i);const B=()=>{!e.paused&&!e.ended&&(window.audioMuted?(e.volume=0,e.muted=!0):(e.muted=!1,F||(e.volume=L),window.backgroundAudio&&!window.backgroundAudio.paused&&U(window.backgroundAudio,0)),y())},O=document.querySelector(".sound-toggle");if(O){O.addEventListener("click",()=>{setTimeout(()=>{B()},50)}),new MutationObserver(m=>{m.forEach(a=>{a.type==="attributes"&&a.attributeName==="class"&&setTimeout(()=>{B()},50)})}).observe(O,{attributes:!0,attributeFilter:["class"]});let r=window.audioMuted;setInterval(()=>{window.audioMuted!==r&&(r=window.audioMuted,B())},500),setTimeout(()=>{B()},1e3)}}export{fe as initVideo};
